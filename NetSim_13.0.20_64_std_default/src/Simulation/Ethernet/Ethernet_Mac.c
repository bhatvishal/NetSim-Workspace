		...
		else
		{
			...
			for (j = 0; j < lan->interfaceCount; j++)
			{
				...
				if (fn_NetSim_MAC_Firewall(d, lan->ethIfVar[j]->interfaceId, p, ACLTYPE_OUTBOUND) == ACLACTION_PERMIT &&
					(lan->ethIfVar[j]->interfaceId != incoming || lan->switchingTechnique == SWITCHINGTECHNIQUE_NULL))
				{
...
static bool decide_move_up(ptrETH_LAN lan, PNETSIM_MACADDRESS destMAC, SWITCHING_TECHNIQUE sw)
{
	if (isBroadcastMAC(destMAC) &&
		sw != SWITCHINGTECHNIQUE_NULL) return false;
...
#pragma warning(default:4189)
	...
	bool isMoveup = decide_move_up(lan, dest, lan->switchingTechnique);
	if (isMoveup)
	{
