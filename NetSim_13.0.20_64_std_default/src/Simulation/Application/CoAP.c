...
int fn_NetSim_Application_ConfigureCOAPTraffic(ptrAPPLICATION_INFO appInfo, void* xmlNetSimNode)
{
	...
	if (xmlChild) {
		...
		getXmlVar(&info->piggybackedTime, PIGGYBACKED_TIME, xmlChild, 0, _INT, COAP);
		getXmlVar(&info->ackResponseTime, ACK_RESPONSE_TIME, xmlChild, 0, _INT, COAP);
		getXmlVar(&info->ackTimeOut, ACK_TIMEOUT, xmlChild, 0, _INT, COAP);
		getXmlVar(&info->ackRandomFactor, ACK_RANDOM_FACTOR, xmlChild, 0, _DOUBLE, COAP);
		getXmlVar(&info->maxRetransmit, MAX_RETRANSMIT, xmlChild, 0, _INT, COAP);
