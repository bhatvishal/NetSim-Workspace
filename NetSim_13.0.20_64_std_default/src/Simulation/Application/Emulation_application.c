...
#define EMULATION_APPLICATION_PROTOCOL_DEFAULT	_strdup("any")
#define EMULATION_APPLICATION_FILTER_STRING_DEFAULT	_strdup("true")
#define EMULATION_APPLICATION_PRIORITY_DEFAULT	10
...
int fn_NetSim_Application_ConfigureEmulationTraffic(ptrAPPLICATION_INFO appInfo,void* xmlNetSimNode)
{
	...
	if(xmlChild)
	{
		...
		getXmlVar(&info->protocol, PROTOCOL, xmlChild, 0, _STRING, EMULATION_APPLICATION);
		getXmlVar(&info->filterString, FILTER_STRING, xmlChild, 0, _STRING, EMULATION_APPLICATION);
		getXmlVar(&info->priority, PRIORITY, xmlChild, 0, _UINT, EMULATION_APPLICATION);
	}
	else
		return 0;
	//Assign other value
