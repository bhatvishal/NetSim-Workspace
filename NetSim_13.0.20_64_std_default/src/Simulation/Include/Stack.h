...
enum enum_MACProtocol
{
	...
	MAC_PROTOCOL_SATELLITE,			// Satellite network, Not really a protocol
};
...
enum enum_InterfaceType
{
	...
	INTERFACE_5G = INTERFACE_LTE_NR, //5G interface
	...
	INTERFACE_P2P,			// Point to Point Interface
};
...
enum enum_DeviceType
{
	...
	SATELLITE_UT=1, //Satellite user terminal
	...
	SATELLITE_GW=6,	//Satellite gateway

	/* 5 layer device without routing or switching protocol*/
	_5G_CORE_AMF = 7,
	_5G_CORE_SMF = 7,
	_5G_CORE_UPF = 7,
...
_declspec(dllexport) void fn_NetSim_Coordinate_ConvertMapToCartesian(double lat, double lon, NetSim_COORDINATES* pos);
_declspec(dllexport) void fn_NetSim_Coordinate_ConvertCartesianToMap(NetSim_COORDINATES* pos);

_declspec(dllexport) void fn_NetSim_Coordinate_ConvertGeoToCartesian(double lat, double lon,
																	 double altitude, char* strRefEllipsoid,
																	 NetSim_COORDINATES* pos);
_declspec(dllexport) void fn_NetSim_Coordinate_ConvertCartesianToGeo(NetSim_COORDINATES* pos,
																	 GEOCORR_REFERENCEELLIPSOID refEllipsoid);
...
_declspec(dllexport) NETSIM_ID fn_NetSim_Stack_GetInterfaceIdByName(NETSIM_ID nDeviceId, const char* szInterfaceName);
...
//Used to find the connected device interface id both ways. Return link id. Costly function to use if network size grows.
_declspec(dllexport) NETSIM_ID fn_NetSim_Stack_GetConnectedInterface_Both(NETSIM_ID d, NETSIM_ID r, NETSIM_ID* din, NETSIM_ID* rin);
...
_declspec(dllexport) void* fn_NetSim_xmlGetNextChildElement(void* xmlNetSimNode, void* prevChild);
...
_declspec(dllexport) DISTRIBUTION fn_NetSim_Config_GetDistribution(const char* szDistribution);
...
_declspec(dllexport) void fn_NetSim_Metrics_Add_new(const NetSim_PACKET* pPacket, int line, char* file, char* function);
...
/**
Used to read freqency and bandwidth.
/param: XmlNode
/Param: name of variable in config file except units. Config file name must be <NAME>_<UNIT>
/Param: default value. Units must be same as return units
/Param: return unit type. Valid units are- GHz, MHz, kHz,Hz
*/
_declspec(dllexport) double fn_NetSim_Config_read_Frequency(void* xmlNetSimNode,
															char* name,
															double defaultval,
															char* returnUnit);

/**
Used to read data rate.
/param: XmlNode
/Param: name of variable in config file except units. Config file name must be <NAME>_<UNIT>
/Param: default value. Units must be same as return units
/Param: return unit type. Valid units are- gbps,mbps,kbps,bps
*/
_declspec(dllexport) double fn_NetSim_Config_read_dataRate(void* xmlNetSimNode,
														   char* name,
...
/Param: name of variable in config file except units. Config file name must be <NAME>_<UNIT>
/Param: default value. Units must be same as return units
/Param: return unit type. Valid units are- GB,MB,KB,B,bits
*/
_declspec(dllexport) double fn_NetSim_Config_read_dataLen(void* xmlNetSimNode,
														  char* name,
														  double defaultDataLen,
														  char* returnUnit);
/**
Used to read transmitter power.
/param: XmlNode
/Param: name of variable in config file except units. Config file name must be <NAME>_<UNIT>
/Param: default value. Units must be same as return units
/Param: return unit type. Valid units are- mW,W,dBm,dB
*/
_declspec(dllexport) double fn_NetSim_Config_read_txPower(void* xmlNetSimNode,
														  char* name,
														  double defaultTxPower,
														  char* returnUnit);

/**
Used to read time/duration.
/param: XmlNode
/Param: name of variable in config file except units. Config file name must be <NAME>_<UNIT>
/Param: default value. Units must be same as return units
/Param: return unit type. Valid units are- Hr(hour),min(minute),sec(second),ms(millisecond),us(microsecond),ns(nanosecond)
*/
...
//Fast emulation
_declspec(dllexport) bool isFastEmulation();

//Virtual Interface
_declspec(dllexport) NETSIM_ID fn_NetSim_Stack_CreateVirtualInterface(NETSIM_ID deviceId, NETSIM_ID parentInterfaceId,
																	  char* virtualInterfaceName,
																	  bool isNwChanged, bool isMacChanged, bool isPhyChanged);
_declspec(dllexport) NETSIM_ID fn_NetSim_Stack_FindVirtualInterface(NETSIM_ID d, char* name);
_declspec(dllexport) bool isVirtualInterface(NETSIM_ID d, NETSIM_ID in);

//Progress report
_declspec(dllexport) void netsim_pause_progress();
_declspec(dllexport) void netsim_resume_progress();
#ifdef  __cplusplus
}
#endif
