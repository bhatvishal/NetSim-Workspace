									   ...
									   ptrOSPFROUTINGTABLEROW row)
{
	...
	if (table->numRow)
		table->rows = realloc(table->rows, ((size_t)table->numRow + 1)*(sizeof* table->rows));
	else
									...
									void* iproute)
{
	if (ospf->ipTableCount)
		ospf->ipTable = realloc(ospf->ipTable, ((size_t)ospf->ipTableCount + 1) * sizeof* ospf->ipTable);
	else
	...
	while (list) {
		for (NETSIM_ID i = 0; i < DEVICE(list->dest_id)->nNumOfInterface; i++)
		{
			if (DEVICE_INTERFACE(list->dest_id, i + 1)->nInterfaceType == INTERFACE_WAN_ROUTER) {
				iptable_add(IP_WRAPPER_GET(ospf->myId),
