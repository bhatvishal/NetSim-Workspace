...
void fn_NetSim_LTENR_RRC_RRCSETUPCOMPLETE_RECV(NETSIM_ID gnbID, NETSIM_ID gnbIF, NETSIM_ID ueID, NETSIM_ID ueIF) {
	...
	if (!fn_NetSim_LTENR_IS_S1_INTERFACE_EXISTS(gnbID)) {
		...
		//ADDITION of SEC cell for option3/3a/3x
		if (pd->isDCEnable && !rrc->isSNSconnected) {
			if (pd->SecCellType == MMWAVE_CELL_TYPE) {
				NETSIM_ID ueInterface = fn_NetSim_FIND_LTENR_INTERFACE(ueID);
				NETSIM_ID SNID = fn_NetSim_LTENR_FIND_BEST_SN_CELL_FOR_UE(ueID, ueInterface);
				if (SNID != 0) {
					NETSIM_ID SNIF = fn_NetSim_LTENR_INTERFACE_ID_FROM_CONNECTED_DEVICE(ueID, ueInterface, SNID);
					fn_Netsim_NSA_Secondary_Cell_Addition(ueID, ueInterface, gnbID, gnbIF, SNID, SNIF);
					rrc->SNID = SNID;
					rrc->SNIF = SNIF;
				}
			}
...
void LTENR_RRCMSG_INIT() {
	...
	nasHandoverTrigger = LTENR_HDR_REGISTER("HANDOVER_COMMAND", NULL, NULL, LTENR_NAS_HANDOVER_COMMAND_FREEHDR, NULL);
	nasHandoverComplete = LTENR_HDR_REGISTER("HANDOVER_COMPLETE", NULL, NULL, NULL, NULL);
	nasPathSwitch = LTENR_HDR_REGISTER("PATH_SWITCH", NULL, NULL, LTENR_NAS_PATH_SWITCH_FREEHDR, NULL);
	naspathSwitchAck = LTENR_HDR_REGISTER("PATH_SWITCH_ACK", NULL, NULL, LTENR_NAS_PATH_SWTICH_ACK_FREEHDR, NULL);
