...
ptrLTENR_GTPU_HEADER fn_NetSim_LTENR_GTPU_DEFAULT_HEADER() {
	ptrLTENR_GTPU_HEADER hdr = calloc(1, sizeof * hdr);
	hdr->V = 1;
	hdr->PT = true;
	hdr->E = false;
	hdr->S = false;
...
#pragma region ADD_GTPU_HEADER_INTO_PACKET
void fn_NetSim_LTENR_ADD_GTPU_HEADER(NetSim_PACKET* packet, NETSIM_ID TEID) {
	ptrLTENR_GTPU_HEADER hdr = fn_NetSim_LTENR_GTPU_DEFAULT_HEADER();
	hdr->TEID = TEID;
	hdr->length = packet->pstruAppData->dPayload + GTPU_HEADER_SIZE;
	//packet->pstruAppData->Packet_AppProtocol = hdr;
}
#pragma endregion

#pragma region REMOVE_GTPU_HEADER_FROM_PACKET
void fn_NetSim_LTENR_REMOVE_GTPU_HEADER(NetSim_PACKET* packet) {
	ptrLTENR_GTPU_HEADER hdr = packet->pstruAppData->Packet_AppProtocol;
	fn_NetSim_LTENR_GTPU_HEADER_FREE(hdr);
}
#pragma endregion