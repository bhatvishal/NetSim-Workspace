...
void fn_NetSim_LTENR_SDAP_AddingQoSInfo() {
	static UINT LTENR_SDAP_QFICOUNT = 1;
	static UINT LTENR_SDAP_ENTITYCOUNT = 1;
...
void fn_NetSim_LTENR_SDAP_HandleMacOut()
{
	...
	ptrLTENR_SDAPVAR var = LTENR_SDAP_GET(d, in);
	if (!isFastProcessing && packet->nPacketType != PacketType_Control) {
	...
	NetSim_PACKET * packet)
{
	...
	NETSIM_ID i = fn_NetSim_Stack_GetInterfaceIdByName(d, "LTE_S1");
	if (i != 0 && i != in) {
		...
		if (!fn_NetSim_GetBufferStatus(buf))
		{
			...
			pevent.nProtocolId = DEVICE_MACLAYER(d, i)->nMacProtocolId;
			fnpAddEvent(&pevent);
		}
...
void fn_NetSim_LTENR_SDAP_HandleMacIn()
{
	...
	if (DEVICE_NWADDRESS(d,in))
		LTENR_SDAP_SENDTONETWORK(d, in, packet);
	else if(!fn_NetSim_LTENR_IS_S1_INTERFACE_EXISTS(d))
		LTENR_GNB_UPF_DATA_PLANE_FORWARDTOOTHERINTERFACE(d, in, packet);
	else
