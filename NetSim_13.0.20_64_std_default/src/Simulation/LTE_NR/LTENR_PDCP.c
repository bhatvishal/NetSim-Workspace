...
NETSIM_ID fn_NetSim_LTENR_PDCP_Splitting_Alogrithm(NETSIM_ID UEID, NETSIM_ID UEIF, ptrLTENR_PDCPENTITY entity) {
	UEIF = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(UEID, UEIF);
	ptrLTENR_UERRC rrc = LTENR_UERRC_GET(UEID, UEIF);
	if (entity->isAlternativePacket) {
		entity->isAlternativePacket = false;
		return rrc->SNID;
	}
	else {
		entity->isAlternativePacket = true;
		return rrc->SelectedCellID;
	}
}

NETSIM_ID fn_NetSim_LTENR_PDCP_Splitting_Alogrithm_GNB(NETSIM_ID d, NETSIM_ID in) {
	in = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(d, in);
...
#pragma region PDCP_PACKET_SATE_VARIABLE
...
void fn_NetSim_LTENR_PDCP_TransmitPDCPSDU()
{
	...
	ptrLTENR_PROTODATA data = LTENR_PROTODATA_GET(entity->ueId, entity->ueIf);
	if (packet->nPacketType != PacketType_Control && data->isDCEnable){
		if (!fn_NetSim_LTENR_NSA_IS_CORE_SPLIT_EXISTS(entity->ueId, entity->ueIf)) {
			if (isUE(d, in)) {
				NETSIM_ID splitID = fn_NetSim_LTENR_PDCP_Splitting_Alogrithm(entity->ueId, entity->ueIf, entity);
				NETSIM_ID UEIF = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(entity->ueId, entity->ueIf);
				ptrLTENR_UERRC rrc = LTENR_UERRC_GET(entity->ueId, UEIF);
				NETSIM_ID prevIN = pstruEventDetails->nInterfaceId;
				if (splitID != packet->nReceiverId) {
					packet->nReceiverId = splitID;
					if(splitID == rrc->SNID)
						pstruEventDetails->nInterfaceId = fn_NetSim_LTENR_SEC_INTEFACE_FOR_UE(entity->ueId, entity->ueIf);
					else if(splitID == rrc->SelectedCellID)
						pstruEventDetails->nInterfaceId = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(entity->ueId, entity->ueIf);
				}
				LTENR_CallRLCOut();
				pstruEventDetails->nInterfaceId = prevIN;
			}
			else if (isGNB(d, in)) {
				NETSIM_ID splitID = fn_NetSim_LTENR_PDCP_Splitting_Alogrithm(entity->ueId, entity->ueIf, entity);
				NETSIM_ID rin = fn_NetSim_LTENR_SEC_INTEFACE_FOR_UE(entity->ueId, entity->ueIf);
				if (splitID != d) {
					packet->nReceiverId = splitID;
					char* gateway = packet->pstruNetworkData->szGatewayIP->str_ip;
					char* nexthop = packet->pstruNetworkData->szNextHopIp->str_ip;
					packet->pstruNetworkData->szGatewayIP = IP_COPY(DEVICE_NWADDRESS(d,
						fn_NetSim_LTENR_CORE_INTERFACE(d, nGC_INTERFACE_XN)));
					packet->pstruNetworkData->szNextHopIp = IP_COPY(DEVICE_NWADDRESS(splitID,
						fn_NetSim_LTENR_CORE_INTERFACE(splitID, nGC_INTERFACE_XN)));
					fn_NetSim_LTENR_SNED_TO_XN_INTERFACE(packet, d, in, entity->ueId, rin, gateway, nexthop, true);
					return;
				}
				else {
					LTENR_CallRLCOut();
			...
			//OPTION 3A, 4A and 7A only pdcp ue split
			if (isUE(d, in)) {
				NETSIM_ID splitID = fn_NetSim_LTENR_PDCP_Splitting_Alogrithm(entity->ueId, entity->ueIf, entity);
				NETSIM_ID UEIF = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(entity->ueId, entity->ueIf);
				ptrLTENR_UERRC rrc = LTENR_UERRC_GET(entity->ueId, UEIF);
...
void fn_NetSim_LTENR_PDCP_ReceivePDCPSDU()
{
	...
	NETSIM_ID rin = fn_NetSim_Get_LTENR_INTERFACE_ID_FROM_DEVICE_ID(r);
	if (!fn_NetSim_LTENR_NSA_IS_CORE_SPLIT_EXISTS(d, in)) {
		if (isGNB(d, in) && packet->nPacketType != PacketType_Control) {
			ptrLTENR_PROTODATA data = LTENR_PROTODATA_GET(r, rin);
			if (data->isDCEnable) {
				rin = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(r, rin);
				ptrLTENR_UERRC rrc = LTENR_UERRC_GET(r, rin);
				NETSIM_ID MCellID = rrc->SelectedCellID;
				if (fn_NetSim_LTENR_NSA_IS_OPTION_X_EXISTS(d, in) && MCellID != d) {
					NETSIM_ID splitID = fn_NetSim_LTENR_PDCP_Splitting_Alogrithm_GNB(r, rin);
					if (splitID != d) {
						char* gateway = packet->pstruNetworkData->szGatewayIP->str_ip;
						char* nexthop = packet->pstruNetworkData->szNextHopIp->str_ip;
						packet->pstruNetworkData->szGatewayIP = IP_COPY(DEVICE_NWADDRESS(d,
							fn_NetSim_LTENR_CORE_INTERFACE(d, nGC_INTERFACE_XN)));
						packet->pstruNetworkData->szNextHopIp = IP_COPY(DEVICE_NWADDRESS(MCellID,
							fn_NetSim_LTENR_CORE_INTERFACE(MCellID, nGC_INTERFACE_XN)));
						fn_NetSim_LTENR_SNED_TO_XN_INTERFACE(packet, d, in, r, rin, gateway, nexthop, false);
						return;
					}
				}
				else {
					if (rrc->SelectedCellID != d) {
		...
		else if (isUE(d, in) && packet->nPacketType != PacketType_Control) {
			ptrLTENR_PROTODATA data = LTENR_PROTODATA_GET(d, in);
			if (data->isDCEnable) {
				in = fn_NetSim_LTENR_MASTER_INTEFACE_FOR_UE(d, in);
				ptrLTENR_UERRC rrc = LTENR_UERRC_GET(d, in);
				if (rrc->SelectedCellID != packet->nTransmitterId)
					packet->nTransmitterId = rrc->SelectedCellID;
			}
		}
	}
